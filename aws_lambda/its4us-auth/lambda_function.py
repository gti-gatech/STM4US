...
def lambda_handler(event, context):
    ...
    '''
    principalId = 'user|' + str(uuid.uuid4())
    ...
    WRITE_PERM = event['stageVariables']['WRITE_PERM']
    READ_PERM = event['stageVariables']['READ_PERM']

    if event['authorizationToken'] == WRITE_PERM:
    ...
    elif event['authorizationToken'] == READ_PERM:
        tmp = event['methodArn'].split(':')
        apiGatewayArnTmp = tmp[5].split('/')
        awsAccountId = tmp[4]
        ...
        policy.denyMethod(HttpVerb.POST, '*')
        policy.denyMethod(HttpVerb.PUT, '*')
        policy.denyMethod(HttpVerb.DELETE, '*')
